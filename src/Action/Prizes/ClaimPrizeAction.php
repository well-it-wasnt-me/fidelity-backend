...
final class ClaimPrizeAction
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        if (empty($prize_id) || !isset($userId)) {
            return $this->responder->withJson($response, ['status' => 'error'])->withStatus(401);
        }
        $prizeClaim = $this->prizesRepository->claimPrize($userId, $prize_id);
