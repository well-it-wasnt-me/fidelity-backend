...
final class TodayMoneyAction
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        ...
        $todaysMoney = $this->statisticsRepository->todaysMoney();

        if ($todaysMoney === false) {
            $todaysMoney = [];
        }

        return $this->responder->withJson($response, $todaysMoney[0]);
    }
}
